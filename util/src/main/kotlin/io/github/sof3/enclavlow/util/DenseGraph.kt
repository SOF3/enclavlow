...
fun <N : Any, E : Edge<E, N>> newDenseGraph(
    nodes: IndexedSet<N> = IndexedSet(),
    newEdge: (MutableDenseGraph<N, E>) -> E,
): MutableDenseGraph<N, E> {
    val edges: MutableList<MutableList<E?>> = MutableList(nodes.size) { MutableList(nodes.size) { null } }
    return MutableDenseGraph(nodes, edges, newEdge)
}
...
sealed class DenseGraph<N : Any, E : Edge<E, N>>(
    nodes: IndexedSet<N>,
    edges: MutableList<MutableList<E?>>,
) {
    open var nodes = nodes; protected set
    open var edges = edges; protected set
    protected abstract var edgeConstructor: (MutableDenseGraph<N, E>) -> E

    override fun equals(other: Any?): Boolean {
        if (other !is DenseGraph<*, *>) return false
    ...
    ) = writeGraphviz(name, graphAttr, nodeAttr, edgeAttr, nodes, ::forEachEdgeIndex)

    fun filterNodes(pred: (N) -> Boolean): MutableDenseGraph<N, E> {
        ...
        return MutableDenseGraph(newNodes, newEdges, edgeConstructor)
    }
    ...
    fun flowsToIndices(dest: N): List<Int> = flowsToIndex(indexOf(dest))
...
/**
 * Directed graph backed by edge matrix
 */
class MutableDenseGraph<N : Any, E : Edge<E, N>>(
    nodes: IndexedSet<N>,
    edges: MutableList<MutableList<E?>>,
    override var edgeConstructor: (MutableDenseGraph<N, E>) -> E,
) : DenseGraph<N, E>(nodes, edges), Cloneable {
    ...
    public override fun clone() = MutableDenseGraph(nodes.clone(), MutableList(nodes.size) { edges[it].toMutableList() }, edgeConstructor)

    infix fun copyTo(target: MutableDenseGraph<N, E>) {
        target.nodes = nodes.clone()
        target.edges = MutableList(nodes.size) { edges[it].toMutableList() }
    }

    fun merge(other: MutableDenseGraph<N, E>, biMap: (E?, E?) -> E?): MutableDenseGraph<N, E> {
        // L
        val out = clone()
...
inline fun <N: Any, E: Edge<E, N>> writeGraphviz(
    name: String,
    graphAttr: List<Pair<String, String>>,
    nodeAttr: (N) -> List<Pair<String, String>>,
    crossinline edgeAttr: (N, N, E) -> Iterable<Pair<String, String>>,
    nodes: IndexedSet<N>,
    forEachEdgeIndex: ((Int, Int, E) -> Unit) -> Unit
) : String {
